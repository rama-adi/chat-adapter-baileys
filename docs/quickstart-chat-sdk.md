...
* `chat` is a **peer dependency** â€” your adapter runs inside the consumer's Chat instance
...
1. A **thread ID interface** â€” the decoded components of your `{adapter}:{segment1}:{segment2}` thread ID
2. A **config interface** â€” credentials and options needed to connect to the platform
...
3. Call `this.chat.processMessage()` with positional args â€” it handles `waitUntil` internally
...
Handle both `EmojiValue` objects and plain strings. `EmojiValue` has a `name` property and `toString()` method â€” there is no `unicode` field.
