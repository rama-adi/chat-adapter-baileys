...
description: "Complete runnable chat-adapter-baileys WhatsApp bot example with common SDK patterns."
---

A complete, runnable example covering the most common patterns. Copy it into your project and run it with `tsx`.
 ...
 * ./auth_info and reused on subsequent runs, so no re-scan is needed.
 *
...
bot.onNewMessage(/.+/, async (thread, message) => {
  // Avoid handling messages posted by this adapter.
  if (message.author.isMe) return;
