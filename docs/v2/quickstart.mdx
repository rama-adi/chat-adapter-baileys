...
description: "Set up a Chat SDK WhatsApp bot with chat-adapter-baileys to receive and reply to messages"
---
...
<Cards>
  <Card title="Concepts" href="/docs/chat-adapter-baileys/v2/concepts">Core concepts for chat-adapter-baileys</Card>
  <Card title="Events and Lifecycle" href="/docs/chat-adapter-baileys/v2/events-and-lifecycle">Adapter events and connection lifecycle</Card>
  <Card title="Thread IDs and Multi-Account" href="/docs/chat-adapter-baileys/v2/thread-ids-and-multi-account">How thread IDs work across multiple accounts</Card>
  <Card title="Formatting and Media" href="/docs/chat-adapter-baileys/v2/formatting-and-media">Send formatted messages and media</Card>
  <Card title="Extensions" href="/docs/chat-adapter-baileys/v2/extensions">Extend adapter behaviour</Card>
  <Card title="Error Handling" href="/docs/chat-adapter-baileys/v2/error-handling">Handle adapter errors gracefully</Card>
  <Card title="Runnable Example" href="/docs/chat-adapter-baileys/v2/example">A complete runnable example</Card>
</Cards>
...
- `state`: the current credentials object, passed into the adapter.
...
<Callout type="warn">
`useMultiFileAuthState` stores credentials as plain JSON files. For production, replace it with a database-backed store (e.g. Redis or Postgres) that implements the same interface.
</Callout>
...
## Create a Chat instance and register handlers
...
<Callout type="warn">
**Register handlers before calling `bot.initialize()` and `adapter.connect()`**, because if you connect first, messages that arrive during handler registration may be missed.
</Callout>
...
bot.onSubscribedMessage(async (thread, message) => {
  if (message.author.isMe) return; // ignore messages sent by this adapter
...
bot.onSubscribedMessage(async (thread, message) => {
  // Avoid echoing messages posted by this adapter.
  if (message.author.isMe) return;
...
See [example.ts](/docs/chat-adapter-baileys/v2/example) for a fuller example with commands, media handling, and reactions.
...
- a **separate auth state**, since each account authenticates independently
...
All handlers (`onNewMention`, `onSubscribedMessage`, etc.) fire for messages from either account. The `thread.id` prefix tells you which account a message came from.

See [Thread IDs and Multi-Account](/docs/chat-adapter-baileys/v2/thread-ids-and-multi-account) for more detail.
...
createBaileysAdapter({
  ...
  // Note: auth and version are managed by the adapter, so don't pass those.
  socketOptions: {},
});
```
...
<Callout type="info">
The adapter manages `auth` and `version` internally, so do not include those in `socketOptions`. Check the [Baileys documentation](https://github.com/WhiskeySockets/Baileys) for all available socket options.
</Callout>
...
| Limitation | Detail |
|---|---|
| `handleWebhook()` returns `501` | Baileys uses an outbound WebSocket, so there are no inbound HTTP webhooks to handle. |
...
You rarely need to construct these manually, as the adapter creates and decodes them for you. See [Thread IDs and Multi-Account](/docs/chat-adapter-baileys/v2/thread-ids-and-multi-account) for details.
