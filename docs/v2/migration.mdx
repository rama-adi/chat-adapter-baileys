...
description: "Upgrade chat-adapter-baileys from v1 to v2 with breaking changes and behavior updates."
---
...
<Cards>
  <Card title="Quickstart" href="/docs/chat-adapter-baileys/v2/quickstart">Setting up a new bot from scratch</Card>
  ...
  <Card title="Extensions" href="/docs/chat-adapter-baileys/v2/extensions">How to use WhatsApp-specific methods in v2</Card>
...
### Replace extension router usage
...
bot.onSubscribedMessage(async (thread, message) => {
  ...
  await wa.markRead(thread.id, [message.id]);
});
```
...
<Callout type="warn">
Do not use older example field names like `event.isAdded` or `event.author`. The current fields are `event.added` (boolean indicating if the reaction was added or removed), `event.user` (the user who reacted), and `event.messageId` (the message that was reacted to).
</Callout>
...
### Review stricter transport validation
...
<Callout type="info">
As of `2.1.0`, `markRead`, `sendLocation`, and `sendPoll` also accept a single named-arguments object. The original positional forms still work, but they are now deprecated and will be removed in the next major version.
</Callout>
