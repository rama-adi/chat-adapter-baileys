...
description: "How thread IDs identify WhatsApp chats and how to handle multiple accounts"
---
...
<Step>
**Its own auth state directory**: a separate `useMultiFileAuthState` folder per account.
</Step>
...
<Step>
**Its own adapter instance**: create one `createBaileysAdapter()` call per account.
</Step>
...
<Callout type="warn">
- `adapterName` must **not** contain `":"` because the adapter validates this at construction time and throws if you pass an invalid name.
