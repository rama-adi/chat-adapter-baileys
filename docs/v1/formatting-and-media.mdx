...
description: "How the Baileys adapter converts text formatting and handles media with WhatsApp."
---
...
<TypeTable
  type={{
    ...
    name: {
      description: "File name (documents use the original filename; others use the type name)",
      type: "string",
    },
    "fetchData()": {
      description: "Downloads the binary content on demand",
      type: "() => Promise<Buffer>",
    },
  }}
/>
...
`fetchData()` downloads the binary content from WhatsApp's media servers. Downloads happen lazily, so nothing is fetched until you call `fetchData()`. The result is a `Buffer`.
