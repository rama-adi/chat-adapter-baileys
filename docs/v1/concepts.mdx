...
description: "Maps Chat SDK concepts like Thread, Channel, and Message to WhatsApp's model."
---
...
The Chat SDK has a `Channel` abstraction for platforms that distinguish between a top-level channel and threads inside it (e.g. Slack channels with message threads). WhatsApp has no such distinction, so every conversation is flat.

Because of this:

- `channelIdFromThreadId()` returns the thread ID unchanged.
- `postChannelMessage()` delegates to `postMessage()`, because posting to the "channel" is the same as posting to the conversation.
- `listThreads()` returns an empty array, as there are no sub-threads to list.
